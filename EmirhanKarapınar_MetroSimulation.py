...
class MetroAgi:
    ...
    def en_az_aktarma_bul(self, baslangic_id: str, hedef_id: str) -> Optional[List[str]]:
        ...
        kuyruk = deque([(baslangic, [baslangic.idx])])
        ziyaret_edildi = set()
    ...
    def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[str], int]]:
        ...
        pq = [(0, baslangic.idx, [baslangic.idx])]
        ziyaret_edildi = {}
        
        while pq:
            toplam_sure, mevcut_id, yol = heapq.heappop(pq)
            mevcut = self.istasyonlar[mevcut_id]  # ID'den istasyonu al
            if mevcut == hedef:
...
if __name__ == "__main__":
    ...
    if rota:
        print("En az aktarmalı rota:", " -> ".join(metro.istasyonlar[i].ad for i in rota))
    
    sonuc = metro.en_hizli_rota_bul("M1", "K4")
    if sonuc:
        rota, sure = sonuc
        print(f"En hızlı rota ({sure} dakika):", " -> ".join(metro.istasyonlar[i].ad for i in rota))
